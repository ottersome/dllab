...
def dSoftmax_d(Z):
    ...
    for col_idx in range(Z.shape[1]):#FOr every sample
        ## Create a jacobian matrix per sample
...
class network:
    ...
    def trainNetwork(self,X,Y):
        ...
        for epoch in range(self.epochs):
            ...
            acc = np.sum(np.argmax(Yhat,axis=0) == np.argmax(Y,axis=0))/Y.shape[1]
            # Evaluate Loss Function
            
            loss =  CatCrossEntropy(Yhat,Y)
            self.backpropagation(Y,Yhat)
            self.updateParams()
            print("At epoch {} we get a loss of {} with accuracy of : {}".format(epoch,loss,acc))
